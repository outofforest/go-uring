...
func makeConns() [MaxConnections]connInfo {
	var conns [MaxConnections]connInfo
	for fd := range conns {
...
func makeBuffers() [][]byte {
	...
	for i := range buffs {
		buffs[i] = make([]byte, MaxConnections)
	}
	return buffs
}
...
func main() {
	...
	if !ring.Params.FastPollFeature() {
		log.Fatal(errors.New("IORING_FEAT_FAST_POLL not available"))
	}
